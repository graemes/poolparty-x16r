/***************************************************************************************************
 * SIMD512 SM3+ CUDA IMPLEMENTATION
 *
 * 	????? - ????
 * 	graemes - 2018
 */
...
int x11_simd512_calc_tpb_64(int thr_id) {
	...
	cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, x11_simd512_gpu_expand_64, 0,	0);

	// calculate theoretical occupancy
	cudaOccupancyMaxActiveBlocksPerMultiprocessor(&maxActiveBlocks, x11_simd512_gpu_expand_64, blockSize, 0);
	cudaGetDevice(&device);
	cudaGetDeviceProperties(&props, device);
	float occupancy = (maxActiveBlocks * blockSize / props.warpSize)
			/ (float) (props.maxThreadsPerMultiProcessor / props.warpSize);

	if (!opt_quiet) gpulog(LOG_INFO, thr_id, "simd512_64 tpb calc - block size %d. Theoretical occupancy: %f", blockSize, minGridSize, occupancy);
