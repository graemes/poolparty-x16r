...
#include "cuda_x11_simd512_func_alexis.cuh"
...
__host__
void x11_simd512_cpu_init_alexis(int thr_id, uint32_t threads){
	cudaMalloc(&d_temp4[thr_id], 64*sizeof(uint4)*threads);
}

__host__
void x11_simd512_cpu_free_alexis(int thr_id){
	cudaFree(d_temp4[thr_id]);
}

__host__
void x11_simd512_cpu_hash_64_alexis(int thr_id, uint32_t threads, uint32_t *d_hash){
