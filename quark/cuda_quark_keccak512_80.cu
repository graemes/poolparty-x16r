...
void KeccakF_80(tKeccakLane * state, const tKeccakLane *in, int laneCount)
{
	while ( --laneCount >= 0 ) {
...
keccak_block_80(uint64_t *s, const uint32_t *in, const uint64_t *keccak_round_constants) {
	size_t i;
...
void quark_keccak512_gpu_hash_80(const uint32_t threads, const uint32_t startNounce, uint64_t *g_hash)
{
	...
	if (thread < threads)
	{
		...
		keccak_block_80(keccak_gpu_state, message, c_keccak_round_constants);
...
void quark_keccak512_setBlock_80(int thr_id, void *pdata)
{
	...
	for ( /* empty */; BLOCKSIZE >= cKeccakR_SizeInBytes; BLOCKSIZE -= cKeccakR_SizeInBytes, in += cKeccakR_SizeInBytes)
	{
		KeccakF_80( state, (const tKeccakLane*)in, cKeccakR_SizeInBytes / sizeof(tKeccakLane) );
	}
	...
	temp[BLOCKSIZE++] = 1;
	memset(temp + BLOCKSIZE, 0, cKeccakR_SizeInBytes - BLOCKSIZE);
	temp[cKeccakR_SizeInBytes-1] |= 0x80;

	// Copy rest of the message in constant memory
	cudaMemcpyToSymbol( c_PaddedMessage, temp, cKeccakR_SizeInBytes, 0, cudaMemcpyHostToDevice);
}

__host__
void quark_keccak512_cuda_hash_80(const int thr_id, const uint32_t threads, const uint32_t startNounce, uint32_t *d_hash, const uint32_t tpb)
{
