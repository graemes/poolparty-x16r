...
#endif
void quark_bmw512_gpu_hash_64(uint32_t threads, uint64_t *g_hash, uint32_t *g_nonceVector)
{
...
void quark_bmw512_gpu_hash_80(uint32_t threads, uint32_t startNounce, uint32_t *resNonce, const uint64_t target)
{
	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
...
__host__
//void quark_bmw512_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_resNonce, const uint32_t tpb)
void quark_bmw512_cpu_hash_80(int thr_id, const uint32_t threads, uint32_t startNounce, uint32_t *d_resNonce, const uint64_t target, const uint32_t tpb)
{
...
__host__
void quark_bmw512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t *d_hash, const uint32_t tpb)
{
...
uint32_t quark_bmw512_calc_tpb_64(int thr_id) {
	...
	if (!opt_quiet) gpulog(LOG_INFO, thr_id, "bmw512_64 tpb calc - block size %d. Theoretical occupancy: %f", blockSize, minGridSize, occupancy);
